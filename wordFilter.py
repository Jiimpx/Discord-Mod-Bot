...
#Inspiration for the filter came from https://pypi.org/project/profanity-filter/
#This is my take on the profanity filter

from collections.abc import Iterable

from constants import ALLOWED_CHARACTERS #Constants library taken from https://pypi.org/project/constants/
from utils import ( #Utils library taken from https://pypi.org/project/python-utils/
...
class Swears:
    #Initalisation function, automatically called when an object is created
    def __init__(self, words=None):
        ...
        self.CENSOR_WORDSET = []
        #Character map, the words are subsituted with the ones in the list and treated as the same character
        self.CHARS_MAPPING = {
            "a": ("a", "@", "*", "4"),
            "i": ("i", "*", "l", "1"),
            "o": ("o", "*", "0", "@"),
        ...
        self._default_wordlist_filename = get_complete_path_of_file(
           ...
           #If no custom file is given load a list of words taken from youtubes own automated blacklist.
           #List found https://www.freewebheaders.com/youtube-blacklist-words-list-youtube-comment-moderation/
           #Scrubbed into a single lined list for use in the bot, I will not be using the default list to show that you can add a custom word list in the filter
        )
        if type(words) == str:
            self.load_censor_words_from_file(words) #Command to add a custom text file to be used in the word filtering, see Discord_moderation_bot.py for more details
        else:
    ...
    def _populate_words_to_wordset(self, words, *, whitelist_words=None):
        ...
        # Validation, checks if the word used is a swear but is whitelisted
        ...
        for word in set(words):
            # All words in CENSOR_WORDSET must be in lowercase for the character map to work
            word = word.lower()
